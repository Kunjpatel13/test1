...
$errors = [];
$success_msg = "";
// Default posted values array (to retain input after submission)
$posted = [
  'first_name' => '',
  'middle_name' => '',
  'last_name' => '',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  foreach ($posted as $k => $v) {
    $posted[$k] = isset($_POST[$k]) ? s($_POST[$k]) : '';
  }

  // Basic required validation
  if ($posted['first_name'] === '') $errors[] = "First name is required.";
  if ($posted['middle_name'] === '') $errors[] = "Middle name is required.";
  if ($posted['last_name'] === '') $errors[] = "Last name is required.";
  if ($posted['email'] === '' || !filter_var($posted['email'], FILTER_VALIDATE_EMAIL)) $errors[] = "Valid email is required.";
  if ($posted['mobile'] === '' || !preg_match('/^[0-9]{10,15}$/', $posted['mobile'])) $errors[] = "Valid mobile number (10-15 digits) is required.";
  if ($posted['study_from'] === '') $errors[] = "Please choose whether you completed UG, PG or Both.";
  if ($posted['highest_qualification'] === '') $errors[] = "Highest qualification is required.";
  if ($posted['city'] === '') $errors[] = "City is required.";
  if ($posted['occupation_type'] === '') $errors[] = "Occupation is required.";

  // dynamic: if study_from == UG or PG or BOTH, validate fields accordingly
  if ($posted['study_from'] === 'UG') {
    if ($posted['ug_course'] === '') $errors[] = "UG course is required.";
    if ($posted['ug_year_join'] === '') $errors[] = "UG year of joining is required.";
    if ($posted['ug_year_pass'] === '') $errors[] = "UG year of passing is required.";
  } elseif ($posted['study_from'] === 'PG') {
    if ($posted['pg_course'] === '') $errors[] = "PG course is required.";
    if ($posted['pg_year_join'] === '') $errors[] = "PG year of joining is required.";
    if ($posted['pg_year_pass'] === '') $errors[] = "PG year of passing is required.";
  } elseif ($posted['study_from'] === 'BOTH') {
    // UG
    if ($posted['ug_course'] === '') $errors[] = "UG course is required.";
    if ($posted['ug_year_join'] === '') $errors[] = "UG year of joining is required.";
    if ($posted['ug_year_pass'] === '') $errors[] = "UG year of passing is required.";
  ...
  // Occupation dynamic
  if ($posted['occupation_type'] === 'Other') {
    if ($posted['occupation_other'] === '') $errors[] = "Please specify your occupation in 'Other'.";
  } else {
    // Job or Business
    if ($posted['company_name'] === '') $errors[] = "Company / Business name is required for Job/Business.";
  }

  // Attendance dynamic
  if ($posted['will_attend'] === 'Yes') {
    if ($posted['members_count'] === '' || !ctype_digit($posted['members_count'])) $errors[] = "Please enter number of members (numeric) if you're attending.";
  } else {
    $posted['members_count'] = null;
  }

  // If no errors -> insert
  if (empty($errors)) {
    $sql = "INSERT INTO alumni_invite (
            first_name, middle_name, last_name, email, mobile, secondary_mobile,
            study_from, ug_course, ug_year_join, ug_year_pass,
            pg_course, pg_year_join, pg_year_pass,
            enrollment_no, highest_qualification, city,
            occupation_type, occupation_other, company_name,
            will_attend, members_count
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if (!$stmt) {
      ...
      $stmt->bind_param(
        ...
        $posted['pg_year_join'],
        $posted['pg_year_pass'],
        $posted['enrollment_no'],
        $posted['highest_qualification'],
        $posted['city'],
        $posted['occupation_type'],
        $posted['occupation_other'],
        $posted['company_name'],
        $posted['will_attend'],
        $posted['members_count']
      );
      $ok = $stmt->execute();
      if ($ok) {
